...
tm_shape(park_boundaries)+tm_polygons(col="forestgreen")
...
tm_shape(land_cover)+tm_raster(style="cat",palette="YlGn")+
  tm_shape(wells_gray)+tm_polygons()
...
plot(land_cover_cropped,col=levels(land_cover_cropped)[[1]]$colour[c(1:10,12,11,13:14)],legend=FALSE)
plot(wells_gray["geometry"],lwd=3,add=TRUE,border="yellow")
plot.new()
legend("left",legend=levels(land_cover_cropped)[[1]]$type[c(1:10,12,11,13:14)],fill=levels(land_cover_cropped)[[1]]$colour[c(1:10,12,11,13:14)],y.intersp=0.85,cex=0.75,pch=22)


labs<-as.character(levels(land_cover_cropped)[[1]]$type)
cols<-levels(land_cover_cropped)[[1]]$colour
tm_shape(land_cover_cropped)+tm_raster(style="cat",palette=cols,labels=labs,title="Land Cover",colorNA=NULL)+
  tm_shape(wells_gray)+tm_borders(col="yellow",lwd=2)+
...
wells_gray_stats<-wells_gray_cover %>%
...
wells_gray_stats %>%
  ggplot(aes(type,sq_km, fill=colour))+
  geom_col(show.legend=FALSE,colour="grey40")+
  scale_fill_identity()+
  coord_flip()+
